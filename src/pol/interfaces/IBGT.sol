...
interface IBGT is IPOLErrors, IERC20, IERC20Metadata {
    ...
    /**
     * @notice Queues a new boost of the validator with an amount of BGT from `source` using delegations.
     * @dev Reverts if `source` does not have enough unboosted balance to cover amount.
     * @dev Reverts if `msg.sender` doesn't have a delegation for the BGT contract from `source`.
     * @param source The delegator allowing `msg.sender` to act on their behalf.
     * @param validator The address of the validator to be boosted.
     * @param amount The amount of BGT to use for the queued boost.
     */
    function queueBoost(address source, address validator, uint128 amount) external;
    ...
    /**
     * @notice Cancels a queued boost of the validator removing an amount of BGT for `source` using delegations.
     * @dev Reverts if `source` does not have enough queued balance to cover amount.
     * @dev Reverts if `msg.sender` doesn't have a delegation for the BGT contract from `source`.
     * @param source The delegator allowing `msg.sender` to act on their behalf.
     * @param validator The address of the validator to cancel boost for.
     * @param amount The amount of BGT to remove from the queued boost.
     */
    function cancelBoost(address source, address validator, uint128 amount) external;
    ...
    /**
     * @notice Boost the validator with an amount of BGT from `source` using delegations.
     * @dev Reverts if `source` does not have enough unboosted balance to cover amount.
     * @dev Reverts if `msg.sender` doesn't have a delegation for the BGT contract from `source`.
     * @param source The delegator allowing `msg.sender` to act on their behalf.
     * @param validator The address of the validator to boost.
     */
    function activateBoost(address source, address validator) external;
    ...
    /**
     * @notice Drops an amount of BGT from an existing boost of validator by `source` using delegations.
     * @param source The delegator allowing `msg.sender` to act on their behalf.
     * @param validator The address of the validator to remove boost from.
     * @param amount The amount of BGT to remove from the boost.
     */
    function dropBoost(address source, address validator, uint128 amount) external;
